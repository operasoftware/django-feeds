...
class TestNaturalDate(unittest.TestCase):
    ...
    def test_seconds_ago(self):
        for n in xrange(0, 59 + 1):
            ...
            self.assertEquals(naturaldate(n_seconds_ago), "just now")

    def test_one_minute_ago(self):
        one_minute_ago = datetime.now() - timedelta(minutes=1)
        self.assertEquals(naturaldate(one_minute_ago), "1 minute ago")
    ...
    def test_days_ago(self):
        ...
        self.assertEquals(naturaldate(yesteryesterday), "2 days ago")

    def test_one_week_ago(self):
        one_week_ago = datetime.now() - timedelta(days=7)
        self.assertEquals(naturaldate(one_week_ago), "1 week ago")

    def test_weeks_ago(self):
        for n in xrange(2, 5):
            n_weeks_ago = datetime.now() - timedelta(days=n * 7)
            self.assertEquals(naturaldate(n_weeks_ago), "%d weeks ago" % n)

    def test_one_month_ago(self):
        one_month_ago = datetime.now() - timedelta(days=30)
        self.assertEquals(naturaldate(one_month_ago), "1 month ago")

    def test_months_ago(self):
        for n in xrange(2, 13):
            n_months_ago = datetime.now() - timedelta(days=n * 30)
            self.assertEquals(naturaldate(n_months_ago), "%d months ago" % n)

    def test_one_year_ago(self):
        one_year_ago = datetime.now() - timedelta(days=365)
        self.assertEquals(naturaldate(one_year_ago), "1 year ago")
